 ...
 *   GGUI_HOST_CREATE_MCP_URL
 *                       Optional domain MCP for the host-create flow
 *                       (host_create_apply + product/safety helpers,
 *                       default http://localhost:6783/mcp). Registers as
 *                       the `host_create` server via the loop below.
...
// (e.g. `GGUI_TODO_MCP_URL` → `todo`, `GGUI_HOST_CREATE_MCP_URL` →
// `host_create`, `GGUI_ORDERS_MCP_URL` → `orders`).
