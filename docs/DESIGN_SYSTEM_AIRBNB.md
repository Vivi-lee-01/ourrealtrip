---
title: OurRealTrip 디자인 시스템 — Airbnb 레퍼런스
date: 2026-05-31
status: reference (적용 대상 — design-system-adoption-spec의 입력)
source: Vivi 제공 (Airbnb design language 추출)
note: 이 문서는 메인 페이지 재설계의 시각 소스 오브 트루스다. 토큰/컴포넌트 매핑은 spec에서 다룬다. 기존 docs/DESIGN_BRIEF.md와의 관계는 PRD에서 정리한다.
---
...
## OurRealTrip 적용 메모 (Vivi 의도)

- 적용 대상 = 로컬 `/Users/vivi/ourrealtrip` (단일 코드베이스). 재배포로 `ourrealtrip-vercel-current.vercel.app`(구버전) 대체.
- "Homes / Experiences / Services" 3-product nav은 Airbnb 고유 구조 — OurRealTrip nav(이벤트/캘린더/탐색 + **이벤트 등록하기**)로 **치환 매핑** 필요(그대로 복제 아님).
- 폰트: Airbnb Cereal VF는 라이선스 폰트라 사용 불가 → **Inter** 대체(문서 명시) 채택.
- merchant 금지 불변식 유지: reservation-card의 "Reserve/결제" 패턴은 OurRealTrip엔 부적용(우리는 결제 안 받음). "참여 신청/관심" 패턴으로 치환.
